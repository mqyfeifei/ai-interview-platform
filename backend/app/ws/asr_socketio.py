...
# ✅ 全局缓存：存储ASR识别结果（voice_id -> {text, timestamp}）
asr_results_cache = {}
...
def handle_audio_chunk(data):
    ...
    client_sid = flask_request.sid  # 保存当前客户端的会话ID
    ...
    try:
        ...
        print(f"[WS] 收到音频分片: voice_id={voice_id}, seq={seq}, is_end={is_end}, chunk_size={len(chunk_b64) if chunk_b64 else 0}, sid={client_sid}")

        state = _ensure_voice_state(voice_id)

        # 保存最新的客户端 sid 到状态，防止重连后闭包内 sid 失效
        state['sid'] = client_sid
        ...
        if chunk_b64:
            ...
            if state['realtime_service'] is None:
                ...
                if not secret_id or not secret_key or not appid:
                    ...
                    socketio.emit('asr_error', {'error': '腾讯云密钥未配置'})  # 广播
                    return
                    ...
                    try:
                        ...
                        if text:
                            ...
                            with asr_results_lock:
                                asr_results_cache[voice_id] = {
                                    'text': text,
                                    'is_final': is_final,
                                    'timestamp': time.time()
                                }
                            print(f"[WS] ✓ 已保存ASR结果到缓存: voice_id={voice_id}, text={text[:30]}")
        ...
        if is_end:
            ...
            # 立即向客户端发送 ack，用于调试（确认后端已收到结束信号）
            try:
                socketio.emit('asr_server_ack', {'voice_id': voice_id, 'msg': 'end_received'})  # 广播
                print(f"[WS] ✓ 已广播 end_ack")
            except Exception as e:
                print(f"[WS] 发送 end_ack 失败: {e}")
        ...
        import traceback
        traceback.print_exc()
        socketio.emit('asr_error', {'error': str(e)})  # 广播


@socketio.on('asr_keepalive')
def handle_asr_keepalive(data):
    """
    保活事件：前端在等待ASR结果时定期发送，防止连接断开
    """
    voice_id = data.get('voice_id', 'unknown')
    from flask import request as flask_request
    client_sid = flask_request.sid
    print(f"[WS] 收到保活信号: voice_id={voice_id}, sid={client_sid}")
    # 不需要做任何事，只是保持连接活跃
