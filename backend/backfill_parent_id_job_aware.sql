...
-- 1) Restore parent_id from backup before recalculation
UPDATE knowledge_tags k
SET parent_id = b.parent_id
FROM knowledge_tags_parent_backup_20260410 b
WHERE b.id = k.id;

-- 2) Recompute parent links with job-aware strategy
WITH parent_tags AS (
    SELECT id, name
    FROM knowledge_tags
...
direct_q_pairs AS (
    SELECT
        ...
        p.id AS parent_id,
        COUNT(*) AS hit_cnt,
        MAX(char_length(p.name)) AS parent_name_len
    FROM child_tags c
    JOIN questions q ON q.reference_answer::text LIKE ('%' || c.name || '%')
...
direct_ranked AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY hit_cnt DESC, parent_name_len DESC, parent_id ASC
        ) AS rn
    FROM direct_q_pairs
),
direct_best AS (
